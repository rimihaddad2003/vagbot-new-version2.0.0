...
module.exports = {
	...
	run: async (client, message) => {
		...
		const [cmd, ...args] = message.content.trim().slice(client.prefix.length).split(/ +/g);
		const command = client.commands.get(cmd.toLowerCase()) || client.commands.get(client.aliases.get(cmd.toLowerCase()));
		if (!command) {
			const best = [...client.commands.map((m) => m.name), ...client.aliases.keys()].filter((c) => leven(cmd.toLowerCase(), c.toLowerCase()) < c.length * 0.4);
			const corr = best.length == 0 ? '' : best.length == 1 ? `Did you mean this : \`${best[0]}\`` : `Did you mean any of these : \`${best.map((v) => v).join('`, `')}\``;
		...
		if (client.cooldowns.has(`${message.author.id}_${command.name}`)) {
			if (client.cooldowns.get(`${message.author.id}_${command.name}`) - Date.now() > 1000) { return message.channel.send(`**⏳ - You have to wait \`${ms(client.cooldowns.get(`${message.author.id}_${command.name}`) - Date.now(), { long: true })}\` before running this command again .**`).then((msg) => msg.delete({ timeout: client.cooldowns.get(`${message.author.id}_${command.name}`) - Date.now() })); }
			else { return message.channel.send(`**⏳ - You have to wait \`${ms(1000, { long: true })}\` before running this command again .**`).then((msg) => msg.delete({ timeout: 1000 })); }
		}
		if (command.args == true && !args.length) return message.channel.send(`**⚠️ - This command require arguments, try \`${client.prefix + cmd.toLowerCase()} ${command.usage}\`**`);
		try {
			await command.run(client, message, args);
			if (command.cooldown) {
				client.cooldowns.set(`${message.author.id}_${command.name}`, Date.now() + command.cooldown);
				setTimeout(() => {
