module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (setting == "event") {
      if (!newset[0]) {
        message.channel.send(
          `**🔍 - Current event role is \`${
            message.guild.roles.cache.get(await client.db.get("event_role"))
              .name
          }\` .**`
        );
      } else {
    ...
    if (setting == "ticket") {
      if (!newset[0]) {
        ...
        await client.db.set("ticket_ch", todb);
        message.channel.send(
    ...
    if (setting == "staff") {
      if (!newset[0]) {
        ...
        await client.db.set("staff_role", todb);
        message.channel.send(
