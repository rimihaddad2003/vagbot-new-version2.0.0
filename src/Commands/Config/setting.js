module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (setting == "family") {
      if (!newset[0]) {
        message.channel.send(
          `**🔍 - Current family role is \`${
          message.guild.roles.cache.get(await client.db.get("fam_role")).name
          }\` .**`
        );
      } else {
        const role =
    ...
    if (setting == "luck") {
      if (!newset[0]) {
        message.channel.send(
          `**🔍 - Current luck role is \`${
          message.guild.roles.cache.get(await client.db.get("luck_role")).name
          }\` .**`
        );
      } else {
        const role =
    ...
    if (setting == "ticket") {
      if (!newset[0]) {
        ...
        message.channel.send(
          `**🔍 - Current tickets category/s: \`${cate.map(value => message.guild.channels.cache.get(value).name).join('`, `')}\` .**`
        );
      } else {
        const todb = [];
        ...
        await client.db.set('ticket_ch',
          todb);
        message.channel.send(
          `**🔍 - Successfully set tickets category/s to: \`${todb.map(value => message.guild.channels.cache.get(value).name).join('`, `')}\` .**`
        );
      }
    }
    if (setting == "staff") {
      if (!newset[0]) {
        ...
        message.channel.send(
          `**🔍 - Current staff role/s: \`${role.map(value => message.guild.roles.cache.get(value).name).join('`, `')}\` .**`
        );
      } else {
        const todb = [];
        newset.forEach(value => {
        ...
        await client.db.set('staff_role',
          todb);
        message.channel.send(
          `**🔍 - Successfully set staff role/s to: \`${todb.map(value => message.guild.roles.cache.get(value).name).join('`, `')}\` .**`
        );
      }}
  },
};